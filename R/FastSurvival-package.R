...
#'     and accumulates the KM product and Greenwood sum in a single scan over
#'     event positions only, without constructing intermediate vectors, making
#'     it approximately 60 times faster than \code{survfit()} plus
#'     \code{summary()} for repeated single-time-point evaluations.}
...
#'     making it approximately 100 times faster than \code{survdiff()}.}
...
#'     The C++ backend performs group splitting, at-risk counting, and
#'     per-distinct-event-time accumulation in a single pass, making it
#'     approximately 100 times faster than \code{coxph()}.}
