...
[`ahrKM()`](https://rdrr.io/pkg/AHR/man/ahrKM.html) from the AHR
package, the reference implementation used by Dormuth et al. (2024).
Because AHR has been archived on CRAN, this benchmark is shown as a
static block rather than a live chunk, so the vignette carries no
undeclared dependency. Install AHR with
