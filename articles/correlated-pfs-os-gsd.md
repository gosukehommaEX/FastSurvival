...
`e.median = list(median_OS_control, median_OS_treatment)` is the latent
OS time, an independent TTP is drawn afterwards and combined with OS to
form the latent PFS. The dropout time is shared between the two
...
#>   Expected analysis time at stop:22.30
```
...
#> 4       Joint power (both endpoints) 0.448
```
...
#> OS_3  0.436 0.522 0.590 0.683 0.843 1.000
```
