...
|----------|----------|------------------------|
| `survfit_fast()` | `survfit()` + `summary()` at one time point | ~50x |
| `survdiff_fast()` | `survdiff()` | ~30x |
| `coxph_fast()` | `coxph()` (point estimate + Wald CI) | ~30x |
| `rmst_fast()` | `survRM2::rmst2()` | ~50x |
| `survdiff_fast(weight = "fh")` | `nph::logrank.test()` | a few hundred x |
